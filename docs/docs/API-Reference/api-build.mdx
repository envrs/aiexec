...
:::info
The `/build` endpoints are used by Aiexec's frontend visual editor code.
These endpoints are part of the internal Aiexec codebase.

Don't use these endpoints to run flows in applications that use your Aiexec flows.
To run flows in your apps, see [Flow trigger endpoints](/api-flows-run).
:::

The `/build` endpoints support Aiexec's frontend code for building flows in the Aiexec visual editor.
You can use these endpoints to build vertices and flows, as well as execute flows with streaming event responses.
You might need to use or understand these endpoints when contributing to the Aiexec codebase.
...
| start_component_id | string | Optional. ID of the component where the execution should start. Component `id` values can be found in [Aiexec JSON files](/concepts-flows-import#aiexec-json-file-contents) |
| stop_component_id | string | Optional. ID of the component where the execution should stop. Component `id` values can be found in [Aiexec JSON files](/concepts-flows-import#aiexec-json-file-contents).|
