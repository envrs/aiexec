...
Retrieve logs for your Aiexec flows and server.

## Enable log retrieval

The `/logs` endpoint requires log retrieval to be enabled in your Aiexec instance.

To enable log retrieval, include set the following [environment variables](/environment-variables) in your Aiexec `.env` file, and then start Aiexec with `uv run aiexec run --env-file .env`:

```text
AIEXEC_ENABLE_LOG_RETRIEVAL=True
AIEXEC_LOG_RETRIEVER_BUFFER_SIZE=10000. // Must be greater than 0
AIEXEC_LOG_LEVEL=DEBUG. // Can be DEBUG, ERROR, INFO, WARNING, or CRITICAL
```
...
```text
{
  ...
  "1736354775699": "2025-01-08T11:46:15.699661-0500 DEBUG File 046-rocket.svg retrieved successfully from flow /Users/USER/Library/Caches/aiexec/profile_pictures/Space.\n"
}
```
