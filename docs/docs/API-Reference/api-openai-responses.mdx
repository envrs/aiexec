...
Aiexec includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses).
...
To be compatible with Aiexec's OpenAI Responses API endpoint, your flow and request must adhere to the following requirements:
...
Aiexec uses `x-api-key` headers for authentication, while OpenAI uses `Authorization: Bearer` headers.
When sending requests to Aiexec with OpenAI client libraries, you must configure custom headers and include an `api_key` configuration.
...
In the following examples, replace the values for `AIEXEC_SERVER_URL`, `AIEXEC_API_KEY`, and `FLOW_ID` with values from your deployment.
<Tabs groupId="client">
...
client = OpenAI(
  base_url="AIEXEC_SERVER_URL/api/v1/",
  default_headers={"x-api-key": "AIEXEC_API_KEY"},
  api_key="dummy-api-key" # Required by OpenAI SDK but not used by Aiexec
)
...
const client = new OpenAI({
  baseURL: "AIEXEC_SERVER_URL/api/v1/",
  defaultHeaders: {
    "x-api-key": "AIEXEC_API_KEY"
  },
  apiKey: "dummy-api-key" // Required by OpenAI SDK but not used by Aiexec
});
...
```bash
curl -X POST \
  "$AIEXEC_SERVER_URL/api/v1/responses" \
...
|--------|----------|-------------|---------|
| `x-api-key` | Yes | Your Aiexec API key for authentication | `"sk-..."` |
| `Content-Type` | Yes | Specifies the JSON format | `"application/json"` |
| `X-AIEXEC-GLOBAL-VAR-*` | No | Global variables for the flow | `"X-AIEXEC-GLOBAL-VAR-API_KEY: sk-..."` For more, see [Pass global variables to your flows in headers](#global-var). |
...
The response contains fields that Aiexec sets dynamically and fields that use OpenAI-compatible defaults.
...
Fields set dynamically by Aiexec:
...
```bash
curl -X POST \
  "http://$AIEXEC_SERVER_URL/api/v1/responses" \
  -H "x-api-key: $AIEXEC_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "model": "$FLOW_ID",
...
The `/responses` endpoint accepts global variables as custom HTTP headers with the format `X-AIEXEC-GLOBAL-VAR-{VARIABLE_NAME}`.
...
This example demonstrates passing an `OPENAI_API_KEY` variable, which is a variable Aiexec automatically detects from environment variables, with two custom variables for `USER_ID` and `ENVIRONMENT`. The variables don't have to be created in Aiexec's Global Variables section - you can pass any variable name in the `X-AIEXEC-GLOBAL-VAR-{VARIABLE_NAME}` header format.
...
curl -X POST \
  ...
  -H "x-api-key: $AIEXEC_API_KEY" \
  -H "Content-Type: application/json" \
  -H "X-AIEXEC-GLOBAL-VAR-OPENAI_API_KEY: sk-..." \
  -H "X-AIEXEC-GLOBAL-VAR-USER_ID: user123" \
  -H "X-AIEXEC-GLOBAL-VAR-ENVIRONMENT: production" \
  -d '{
    "model": "your-flow-id",
    "input": "Hello"
  }'
...
Variables passed with `X-AIEXEC-GLOBAL-VAR-{VARIABLE_NAME}` are always available to your flow, regardless of whether they exist in the database.

If your flow components reference variables that aren't provided in headers or your Aiexec database, the flow fails by default.
