---
title: Get started with the Aiexec API
...
You can use the Aiexec API for programmatic interactions with Aiexec, such as the following:

* Create and edit flows, including file management for flows.
* Develop applications that use your flows.
* Develop custom components.
* Build Aiexec as a dependency of a larger application, codebase, or service.
* Contribute to the overall Aiexec codebase.

To view and test all available endpoints, you can access the Aiexec API's OpenAPI specification at your Aiexec deployment's `/docs` endpoint, such as `http://localhost:7860/docs`.

:::tip Try it
For an example of the Aiexec API in a script, see the [Aiexec quickstart](/get-started-quickstart).
...
## Form Aiexec API requests

While individual options vary by endpoint, all Aiexec API requests share some commonalities, like a URL, method, parameters, and authentication.

As an example of a Aiexec API request, the following curl command calls the `/v1/run` endpoint, and it passes a runtime override (`tweaks`) to the flow's **Chat Output** component:

```bash
curl --request POST \
  --url "$AIEXEC_SERVER_URL/api/v1/run/$FLOW_ID?stream=false" \
...
By default, local deployments serve the Aiexec API at `http://localhost:7860/api`.

Remotely hosted Aiexec deployments are available at the domain set by the hosting service, such as `http://IP_OR_DNS/api` or `http://IP_OR_DNS:AIEXEC_PORT/api`.

You can configure the Aiexec port number in the `AIEXEC_PORT` [environment variable](/environment-variables).

* `https://UUID.ngrok.app/api`
* `http://IP_OR_DNS/api`
* `http://IP_OR_DNS:AIEXEC_PORT/api`

### Authentication

In Aiexec versions 1.5 and later, most API endpoints require authentication with a Aiexec API key in either an `x-api-key` header or query parameter.
...
Aiexec API requests use various methods, paths, path parameters, query parameters, and body parameters.
...
The Aiexec API serves `/v1` and `/v2` endpoints.
...
export AIEXEC_SERVER_URL="https://localhost:7860"
...
Commonly used values in Aiexec API requests include your [Aiexec server URL](#base-url), [Aiexec API keys](#authentication), flow IDs, and [project IDs](/api-projects#read-projects).
...
## Try some Aiexec API requests

Once you have your Aiexec server URL, try calling these endpoints that return Aiexec metadata.

### Health check

Returns the health status of the Aiexec database and chat services:

```bash
curl -X GET \
  "$AIEXEC_SERVER_URL/health_check" \
...
Aiexec provides an additional `GET /health` endpoint.
This endpoint is served by uvicorn before Aiexec is fully initialized, so it's not reliable for checking Aiexec service health.

### Get version

Returns the current Aiexec API version:

```bash
curl -X GET \
  "$AIEXEC_SERVER_URL/api/v1/version" \
  -H "accept: application/json"
```
...
Returns configuration details for your Aiexec deployment:

```bash
curl -X GET \
  "$AIEXEC_SERVER_URL/api/v1/config" \
  -H "accept: application/json"
```
...
Returns a dictionary of all Aiexec components.
Requires a [Aiexec API key](/api-keys-and-authentication).

```bash
curl -X GET \
  "$AIEXEC_SERVER_URL/api/v1/all" \
  -H "accept: application/json" \
...
Because you can run Aiexec as either an IDE (frontend and backend) or a runtime (headless, backend-only), it serves endpoints that support frontend and backend operations.
Many endpoints are for orchestration between the frontend and backend, reading and writing to the Aiexec database, or enabling frontend functionality, like the **Playground**.
Unless you are contributing to the Aiexec codebase, you won't directly call most of the Aiexec endpoints.
...
The following endpoints are useful for developing applications with Aiexec and administering Aiexec deployments with one or more users.
...
* Deployment details:
  * GET `/v1/version`: Return Aiexec version. See [Get version](/api-reference-api-examples#get-version).
...
* Custom components: You might use these endpoints when developing custom Aiexec components for your own use or to share with the Aiexec community:
  * GET `/v1/all`: Return all available Aiexec component types. See [Get all components](/api-reference-api-examples#get-all-components).
...
The following endpoints are most often used when contributing to the Aiexec codebase, and you need to understand or call endpoints that support frontend-to-backend orchestration or other internal functionality.
...
* MCP servers: The following endpoints are for managing Aiexec MCP servers and MCP server connections.
They aren't typically called directly; instead, they are used to drive internal functionality in the Aiexec frontend and when running flows that call MCP servers.
...
* Use the Aiexec API to [run a flow](/api-flows-run).
* Use the Aiexec API to [upload files](/api-files).
* Use the Aiexec API to [get flow logs](/api-logs).
* Explore all endpoints in the [Aiexec API specification](/api).