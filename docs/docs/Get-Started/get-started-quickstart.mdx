...
Get started with Aiexec by loading a template flow, running it, and then serving it at the `/run` API endpoint.
...
- Create a [Aiexec API key](/api-keys-and-authentication)
    ...
    <details>
    <summary>Create a Aiexec API key</summary>

    A Aiexec API key is a user-specific token you can use with Aiexec.

    To create a Aiexec API key, do the following:

    1. In Aiexec, click your user icon, and then select **Settings**.
    2. Click **Aiexec API Keys**, and then click <Icon name="Plus" aria-hidden="true"/> **Add New**.
    3. Name your key, and then click **Create API Key**.
    4. Copy the API key and store it securely.
    5. To use your Aiexec API key in a request, set a `AIEXEC_API_KEY` environment variable in your terminal, and then include an `x-api-key` header or query parameter with your request.
    For example:
...
Aiexec is an IDE, but it's also a runtime you can call through the [Aiexec API](/api-reference-api-examples) with Python, JavaScript, or HTTP.

When you start Aiexec locally, you can send requests to the local Aiexec server.
For production applications, you need to [deploy a stable Aiexec instance](/deployment-overview) to handle API calls.

For example, you can use the `/run` endpoint to run a flow and get the result.

Aiexec provides code snippets to help you get started with the Aiexec API.

1. When editing a flow, click **Share**, and then click **API access**.

    The default code in the API access pane constructs a request with the Aiexec server `url`, `headers`, and a `payload` of request data.
    The code snippets automatically include the `AIEXEC_SERVER_ADDRESS` and `FLOW_ID` values for the flow, and a script to include your `AIEXEC_API_KEY` if you've set it as an environment variable in your terminal session.
    ...
    url = "http://AIEXEC_SERVER_ADDRESS/api/v1/run/FLOW_ID"  # The complete API endpoint URL for this flow
    ...
    fetch('http://AIEXEC_SERVER_ADDRESS/api/v1/run/FLOW_ID', options)
    ...
    ```bash
    curl --request POST \
         --url 'http://AIEXEC_SERVER_ADDRESS/api/v1/run/FLOW_ID?stream=false' \
         --header 'Content-Type: application/json' \
         --header "x-api-key: AIEXEC_API_KEY" \
         --data '{
          "output_type": "chat",
...
In a production application, you probably want to select parts of this response to return to the user, store in logs, and so on. The next steps demonstrate how you can extract data from a Aiexec API response to use in your application.
...
This script runs a question-and-answer chat in your terminal and stores the agent's previous answer so you can compare them.
    ...
    const url = 'http://AIEXEC_SERVER_ADDRESS/api/v1/run/FLOW_ID';
...
To assist with formatting, you can define tweaks in Aiexec's **Input Schema** pane before copying the code snippet.
...
Aiexec updates the `tweaks` object in the code snippets based on your input parameters, and includes default values to guide you.
...
* [Trigger flows with the Aiexec API](/concepts-publish)
* [Use Aiexec as a Model Context Protocol (MCP) server](/mcp-server)
