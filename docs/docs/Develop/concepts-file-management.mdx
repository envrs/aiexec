...
Each Aiexec server has a file management system where you can store files that you want to use in your flows.

Files uploaded to Aiexec file management are stored locally in your [Aiexec configuration directory](/memory), and they are available to all of your flows.
Local storage is set by `AIEXEC_STORAGE_TYPE`, which has only one allowed value (`local`).

Uploading files to Aiexec file management keeps your files in a central location, and allows you to reuse files across flows without repeated manual uploads.

## Use the file management UI

You can use the file management UI to upload files from your local machine to your own Aiexec server.
You can also manage all files that have been uploaded to your Aiexec server.

1. Navigate to Aiexec file management:

    * **Aiexec Desktop**: In Aiexec, on the [**Projects** page](/concepts-flows#projects) page, click **My Files** below the list of projects.
    * **Aiexec OSS**: From a browser, navigate to your Aiexec server's `/files` endpoint, such as `http://localhost:7860/files`. Modify the base URL as needed for your Aiexec server.
    * **Backend-only**: For programmatic file management, use the [Aiexec API files endpoints](/api-files). However, the following steps assume you're using the file management UI.
...
## Upload and manage files with the Aiexec API

With the Aiexec API, you can upload and manage files in Aiexec file management, and you can send files to flows programmatically at runtime.
...
To modify this value, change the `AIEXEC_MAX_FILE_SIZE_UPLOAD` [environment variable](/environment-variables).

## Use files in a flow

To use files in your Aiexec file management system in a flow, add a component that accepts file input to your flow, such as the **File** component.
...
3. Expand the **File** section, find the **Files** row, and then enable **Expose Input** to allow the parameter to be set at runtime through the Aiexec API.
...
5. When you run this flow programmatically, your script must upload a file to Aiexec file management, and then pass the returned `file_path` to the `path` tweak in the `/run` request:
...
Aiexec supports base64 images in the following formats:
...
You can upload images to the **Playground** chat interface and as runtime input with the Aiexec API.
...
* When you trigger a flow with the `/api/v1/run/$FLOW_ID` endpoint, you can use the `files` parameter to attach image data as a base64-encoded string:
   ...
   ```bash
   curl -X POST "http://$AIEXEC_SERVER_ADDRESS/api/v1/run/$FLOW_ID" \
