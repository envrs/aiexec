...
:::info
Voice mode is not available in Aiexec Desktop.
To use voice mode, [Install the Aiexec OSS Python package](/get-started-installation#install-and-run-the-langflow-oss-python-package).
You can use Aiexec's voice mode to interact with your flows verbally through a microphone and speakers.
...
* An [OpenAI](https://platform.openai.com/) account and an OpenAI API key because Aiexec uses the OpenAI API to process voice input and generate responses.
...
5. Enter your OpenAI API key, and then click **Save**. Aiexec saves the key as a [global variable](/configuration-global-variables).
...
Aiexec saves this key as a global variable.
...
Aiexec exposes two OpenAI Realtime API-compatible websocket endpoints for your flows.
...
The Aiexec API's websockets endpoints require an [OpenAI API key](https://platform.openai.com/docs/overview) for authentication, and they support an optional [ElevenLabs](https://elevenlabs.io) integration with an ElevenLabs API key.
...
This is the mode used in the Aiexec **Playground**.
...
If omitted, Aiexec uses the flow ID as the [session ID](/session-id).
