...
Replace `AIEXEC_SERVER_ADDRESS`, `FLOW_ID`, and `AIEXEC_API_KEY` with the values from your Aiexec deployment.
```
curl --request POST \
...
To retrieve messages from local Aiexec memory, add a [**Message History** component](/components-helpers#message-history) to your flow.
...
Messages can be retrieved by `session_id` from the Aiexec API at `GET /v1/monitor/messages`. For more information, see [Monitor endpoints](https://aiexec-docs.khulnasoft.com/api-monitor).
