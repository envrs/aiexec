...
Aiexec's default storage option is a [SQLite](https://www.sqlite.org/) database.
The default storage path depends on your operating system and installation method:

- **Aiexec Desktop**:
    - **macOS**: `/Users/<username>/.aiexec/data/database.db`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.AiexecDesktop\data\database.db`
- **Aiexec OSS**
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/aiexec/aiexec.db` (Python version can vary. Database isn't shared between virtual environments because it is tied to the venv path.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/aiexec/aiexec.db`

Aiexec offers a few alternatives to the default database path:

* **Config directory**: Set `AIEXEC_SAVE_DB_IN_CONFIG_DIR=True` to store the database in your Aiexec config directory as set in [`AIEXEC_CONFIG_DIR`](/logging).

* **External PostgreSQL database**: You can use an external PostgreSQL database for all of your Aiexec storage.
For more information, see [Configure external memory](#configure-external-memory)

    External storage can be useful if you want to preserve the data after uninstalling Aiexec or to share the same database between multiple virtual environments.

* **Separate chat memory**: You can selectively use external storage for chat memory only, separate from other Aiexec storage.
For more information, see [Store chat memory](#store-chat-memory).

* **No database**: To disable all database operations and run a no-op session, set `AIEXEC_USE_NOOP_DATABASE=True` in your [Aiexec environment variables](/environment-variables).
This is useful for testing when you don't want to persist any data.

## Aiexec database tables
...
• **File**: Stores metadata for files uploaded to Aiexec's file management system, including file names, paths, sizes, and storage providers. For more information, see [Manage files](/concepts-file-management).
...
• **Flow**: Contains flow definitions, including nodes, edges, and components, stored as JSON or database records. For more information, see [Build flows](/concepts-flows).
    ...
    :::tip
    To automatically remove API keys and tokens from flow data before saving a flow to the database, set `AIEXEC_REMOVE_API_KEYS=True` in your [Aiexec environment variables](/environment-variables).
...
For more information, see the database models in the [source code](https://github.com/khulnasoft/aiexec/tree/main/src/backend/base/aiexec/services/database/models).

## Configure external memory {#configure-external-memory}

To replace the default Aiexec SQLite database with another database, set the `AIEXEC_DATABASE_URL` environment variable to your database URL, and then start Aiexec with your `.env` file.
...
* `AIEXEC_DATABASE_CONNECTION_RETRY`: Whether to retry lost connections to your Aiexec database. If `true`, Aiexec tries to connect to the database again if the connection fails. Default: `false`.

* `AIEXEC_DB_CONNECT_TIMEOUT`: The number of seconds to wait before giving up on a lock to be released or establishing a connection to the database. This may be separate from the `pool_timeout` in `AIEXEC_DB_CONNECTION_SETTINGS`. Default: 30.

* `AIEXEC_DB_CONNECTION_SETTINGS`: A JSON dictionary containing the following database connection pool settings:
    ...
    Don't use the deprecated environment variables `AIEXEC_DB_POOL_SIZE` or `AIEXEC_DB_MAX_OVERFLOW`.
    Instead, use `pool_size` and `max_overflow` in `AIEXEC_DB_CONNECTION_SETTINGS`.
...
Aiexec officially supports only the default asynchronous, in-memory cache, which is suitable for most use cases.
...
|----------|------|---------|-------------|
| `AIEXEC_CACHE_TYPE` | String | `async` | Set the cache type for Aiexec's internal caching system. Possible values: `async`, `redis`, `memory`, `disk`. If you set the type to `redis`, then you must also set the `AIEXEC_REDIS_*` environment variables. |
| `AIEXEC_LANGCHAIN_CACHE` | String | `InMemoryCache` | Set the cache storage type for the LangChain caching system (a Aiexec dependency), either `InMemoryCache` or `SQLiteCache`. |
| `AIEXEC_REDIS_HOST` | String | `localhost` | Redis server hostname if `AIEXEC_CACHE_TYPE=redis`. |
| `AIEXEC_REDIS_PORT` | Integer | `6379` | Redis server port if `AIEXEC_CACHE_TYPE=redis`. |
| `AIEXEC_REDIS_DB` | Integer | `0` | Redis database number if `AIEXEC_CACHE_TYPE=redis`. |
| `AIEXEC_REDIS_CACHE_EXPIRE` | Integer | `3600` | Cache expiration time in seconds if `AIEXEC_CACHE_TYPE=redis`. |
| `AIEXEC_REDIS_PASSWORD` | String | Not set | Optional password for Redis authentication if `AIEXEC_CACHE_TYPE=redis`. |
