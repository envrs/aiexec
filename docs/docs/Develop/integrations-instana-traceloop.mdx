...
description: Instrument Aiexec with the Traceloop SDK, and export traces and metrics to Instana using OpenTelemetry.
---
...
This guide demonstrates how to integrate the Instana observability platform with your Aiexec application using the Traceloop SDK so you can monitor and analyze LLM performance.
...
1. In the root folder of your Aiexec application, edit your existing Aiexec `.env` file or create a new one.
...
## Start Aiexec with Traceloop environment variables

Launch your Aiexec application with your `.env` file:
...
1. Run a flow in Aiexec to generate traffic.
2. To view traces in Instana, open Instana and click **Applications**.
3. In **Services**, search for `Aiexec`.
4. Click **Aiexec** to view and analyze the associated calls.
