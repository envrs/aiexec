...
In contrast, [environment variables](/environment-variables), like `AIEXEC_PORT` or `AIEXEC_LOG_LEVEL`, are generally for broader settings that configure how Aiexec runs.
However, Aiexec can also source global variables from environment variables.

Aiexec stores global variables in its internal database, and it encrypts the values using a secret key.
...
5. Optional: Select a **Type** for your global variable. The available types are **Generic** (default) and **Credential**.
   ...
   Aiexec encrypts both **Generic** and **Credential** type global variables.
...
Aiexec can source custom global variables from your runtime environment.
For information about how Aiexec detects and applies environment variables, see [Aiexec environment variables](/environment-variables).

Aiexec automatically generates global variables based on [`constants.py`](https://github.com/khulnasoft/aiexec/blob/main/src/wfx/src/wfx/services/settings/constants.py) if it detects any matching environment variables.
For example, if you set `OPENAI_API_KEY` in your runtime environment, Aiexec automatically generates a global variable using that value.

You can declare additional variables in `AIEXEC_VARIABLES_TO_GET_FROM_ENVIRONMENT`.
For example, `AIEXEC_VARIABLES_TO_GET_FROM_ENVIRONMENT=WATSONX_PROJECT_ID,WATSONX_API_KEY` creates global variables named `WATSONX_PROJECT_ID` and `WATSONX_API_KEY` in Aiexec's database.
Then, you can use these variables wherever they are needed in your component settings.

<Tabs>
<TabItem value="local" label="Local" default>

If you installed Aiexec locally, set `AIEXEC_VARIABLES_TO_GET_FROM_ENVIRONMENT` in your Aiexec `.env` file:

1. Create or edit your Aiexec `.env` file.
...
If you're using Docker, there are two ways that you can set `AIEXEC_VARIABLES_TO_GET_FROM_ENVIRONMENT`:
...
The list in `AIEXEC_VARIABLES_TO_GET_FROM_ENVIRONMENT` includes only the variable names.
You must ensure that these environment variables are defined in your Docker environment, such as with `-e` or otherwise.

</TabItem>
</Tabs>

After starting Aiexec, go to your Aiexec **Settings** to confirm that the variables were created.

Only the **Name** and **Value** are taken from the environment.
You can edit the variables in your Aiexec **Settings** if you want to configure additional options, such as the **Apply To Fields** option.
...
However, Aiexec automatically encrypts _all_ global variables stored in the database.
...
If you want to automatically set fallback values for your global variables to environment variables, set `AIEXEC_FALLBACK_TO_ENV_VAR=True` in your `.env` file.
When this setting is enabled, if a global variable isn't found, Aiexec attempts to use an environment variable with the same name as a backup.

For example, assume you have the following Aiexec `.env` configuration, and your flow has a component that expects a `WATSONX_API_KEY` global variable:
...
When you run the flow, if there is no global variable named `WATSONX_API_KEY`, Aiexec looks for an environment variable named `WATSONX_API_KEY`.
In this example, Aiexec uses the `WATSONX_API_KEY` value from the `.env` to run the flow.