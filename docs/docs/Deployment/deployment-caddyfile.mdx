...
3. Add the following values to `docker-compose.yml`, and then save the file.
    ...
    The following example defines the Aiexec service from the `aiexec:latest` image and a Caddy service to expose Aiexec through a reverse proxy.
...
5. Add the following values to `Caddyfile`, and then save the file.
    ...
    This Caddyfile configures Caddy to listen on port 80, and forward all incoming requests to port 80 to the Aiexec service at port 7860.
...
7. To access your Aiexec server over the public internet, navigate to your server's public IP address, such as `http://5.161.250.132`.
...
8. Recommended: Enable HTTPS:
    ...
    3. Modify your Caddyfile to include port `443` so Caddy can forward both HTTP (port 80) and HTTPS (port 443) requests to the Aiexec service:
...
To package your local flows into a custom Docker image, see [Containerize a Aiexec application](/develop-application).
