---
title: Deploy Aiexec with Nginx and SSL
slug: /deployment-nginx-ssl
---

import Icon from "@site/src/components/icon";

Deploy Aiexec on a Linux-based server using Nginx as a reverse proxy, Let's Encrypt for SSL certificates, and Certbot for automated certificate management.

This setup encrypts all communications between users and your Aiexec server. SSL certificates ensure that sensitive data is protected from eavesdropping and tampering, and the automatic certificate management through Certbot eliminates the complexity of manual SSL configuration.
...
## Install Aiexec on your server

To install Aiexec on your server, do the following:
...
3. Install uv to manage Python packages.
    Aiexec recommends uv for faster installation.
    ```bash
    pip install uv
    ```

4. Create a virtual environment for Aiexec.
    ```bash
    uv venv aiexec-venv
    source aiexec-venv/bin/activate
    ```

5. Install Aiexec using uv.
    ```bash
    uv pip install aiexec
    ```

6. Optionally, start Aiexec.
    ```bash
    uv run aiexec run --host 127.0.0.1 --port 7860 &
    ```

    Test access at your `http://YOUR_PUBLIC_IP:7860`.
    Since Aiexec is running on `localhost`, you should not be successful.
...
Nginx is a reverse proxy that receives external requests and forwards them to your Aiexec server. In addition to SSL termination, Nginx includes features for load balancing and security. For more information, see the [Nginx documentation](https://nginx.org/en/docs/).
...
Replace **DOMAIN_NAME** with your actual domain name, such as `aiexec.example.com`.
    ```bash
...
3. Start Aiexec from the virtual environment.
    ...
    b. Start Aiexec in the background.
    ```bash
    uv run aiexec run --host 127.0.0.1 --port 7860 &
    ```
...
- [Deploy Aiexec on a remote server with Caddy](/deployment-caddyfile)
