...
The [Aiexec runtime Helm chart](https://github.com/khulnasoft/aiexec-helm-charts/blob/main/charts/aiexec-runtime) is tailored for deploying applications in a production environment. It is focused on stability, performance, isolation, and security to ensure that applications run reliably and efficiently.

:::warning
For security reasons, the default Aiexec runtime Helm chart sets [`readOnlyRootFilesystem: true`](https://github.com/khulnasoft/aiexec-helm-charts/blob/main/charts/aiexec-runtime/values.yaml#L46). This setting prevents modifications to the container's root filesystem at runtime, which is a recommended security measure in production environments.
...
2. Install the Aiexec app with the default options in the `aiexec` namespace.
    ...
    If you have a [custom image with packaged flows](/deployment-docker#package-your-flow-as-a-docker-image), you can deploy Aiexec by overriding the default [`values.yaml`](https://github.com/khulnasoft/aiexec-helm-charts/blob/main/charts/aiexec-runtime/values.yaml) with the `--set` flag:
...
Use the `.env` section of the Aiexec runtime Helm chart's [`values.yaml`](https://github.com/khulnasoft/aiexec-helm-charts/blob/main/charts/aiexec-runtime/values.yaml) file to define environment variables for your Aiexec deployment.
...
Use `replicaCount` and `resources` in the Aiexec runtime Helm chart's [`values.yaml`](https://github.com/khulnasoft/aiexec-helm-charts/blob/main/charts/aiexec-runtime/values.yaml) file to configure scaling:
