...
For all changes, see the [Changelog](https://github.com/khulnasoft/aiexec/releases/latest).
...
For all changes, see the [Changelog](https://github.com/khulnasoft/aiexec/releases).

### Known issue, potential security vulnerability: .env file not loaded in versions 1.6.0 through 1.6.3 {#env-file-bug}

Aiexec versions 1.6.0 through 1.6.3 have a critical bug where environment variables from `.env` files aren't read.
...
If your `.env` file includes `AUTO_LOGIN=false`, upgrading to the impacted versions causes Aiexec to fall back to default settings, potentially giving all users superuser access immediately upon upgrade.
Additionally, database credentials, API keys, and other sensitive configurations can't be loaded from `.env` files.

_Don't_ upgrade to any Aiexec version from 1.6.0 through 1.6.3 if you use `.env` files for configuration.
...
:::warning
Windows users of Aiexec Desktop should **not** use the in-app update feature to upgrade to Aiexec version 1.6.0.
Only Windows Desktop users upgrading to Aiexec version 1.6.0 are affected.
:::

The **Update** button in Aiexec Desktop will not work for Windows users, and may result in data loss.

Instead, download a fresh installation from the [Aiexec website](https://aiexec.khulnasoft.com) when a new version is available.

Follow the instructions below to minimize the risk of losing flows.

These instructions assume the default path for the Aiexec database of `C:\Users\YOUR_USERNAME\AppData\Roaming\com.aiexec\data\database.db` or `C:\Users\%YOUR_USERNAME%\AppData\Roaming\com.aiexec\data\database.db`. Replace `YOUR_USERNAME` with your Windows username.

1. Ensure you have Administrator privileges.
2. Ensure you have enough disk space for a second installation of Aiexec.
3. Close Aiexec, and ensure no Aiexec process is running in Task Manager.
4. The Aiexec 1.6.0 installer automatically performs a database backup in a later step, but an additional manual backup provides additional redundancy and is recommended.
To manually back up your Aiexec database file, do the following:
   1. In Windows Explorer, navigate to `C:\Users\YOUR_USERNAME\AppData\Roaming\com.aiexec\data\`. Replace `YOUR_USERNAME` with your Windows username.
   2. Copy the `database.db` at this location, and paste it to a safe location.
5. Download the Windows installer from the [Aiexec website](https://aiexec.khulnasoft.com).
...
The Aiexec 1.6.0 installer automatically performs a database backup.
These steps install Aiexec from scratch, and result in two Aiexec installations: the previously installed version, and version 1.6.0.
This is expected behavior.
8. Start version 1.6.0 of Aiexec, and confirm your flows behave as expected.
9. **If flows are missing**, restore your flows from your manual backup by doing the following:
    ...
    4. Start Aiexec, and confirm your flows behave as expected.
    5. After confirmation, uninstall the previous version of Aiexec, and keep version 1.6.0.

### Breaking changes

- Authentication enforced for Aiexec API requests by default

    In Aiexec version 1.6, `AIEXEC_AUTO_LOGIN=True` and `AIEXEC_SKIP_AUTH_AUTO_LOGIN=False` by default.
    This enforces authentication for Aiexec API requests while still automatically authenticating all users as superusers in the visual editor.
    ...
    **This is a breaking change from 1.5 where both of these environment variables were `true` by default, bypassing all authentication.**
    ...
    For more information, see the documentation for [`AIEXEC_AUTO_LOGIN`](/api-keys-and-authentication#aiexec-auto-login).

### New features and enhancements

- OpenAI Responses API compatibility

  Aiexec now includes an endpoint that is compatible with the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses) at `POST /api/v1/responses`.
...
- Advanced document parsing with built-in Docling support
  ...
  To make it easier to use the [**Docling** components](/bundles-docling) and the **File** component's new advanced parsing feature, the Docling dependency is now included with Aiexec for all operating systems except macOS Intel (x86_64).
...
- Aiexec API requests can require authentication

    To enhance security and ensure proper authentication for automatic login features, Aiexec API endpoints now require authentication with a Aiexec API key, even when `AIEXEC_AUTO_LOGIN=True`.
    This change will be enforced in a future release.
    For temporary backwards compatibility, this release adds the `AIEXEC_SKIP_AUTH_AUTO_LOGIN` environment variable.
    The default value is `true`, which disables API authentication enforcement.
    To enforce API authentication, set `AIEXEC_SKIP_AUTH_AUTO_LOGIN=False`.
    For more information, see the documentation for [`AIEXEC_AUTO_LOGIN`](/api-keys-and-authentication#aiexec-auto-login).
...
- Stability improvements
    ...
    See an issue? [Raise it on GitHub](https://github.com/khulnasoft/aiexec/issues).
...
See the [Changelog](https://github.com/khulnasoft/aiexec/releases).