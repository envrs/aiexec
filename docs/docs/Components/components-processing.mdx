...
2. In **File Name**, enter a file name and an optional path.
    ...
    * **Default location**: If you only provide a file name, then the file is stored in the Aiexec data directory. For example,`~/Library/Caches/aiexec/data` on macOS.
...
4. To test the **Save File** component, click <Icon name="Play" aria-hidden="true" /> **Run component**, and then click <Icon name="TextSearch" aria-hidden="true" /> **Inspect output** to get the filepath where the file was saved.
    ...
    ```text
    DataFrame saved successfully as 'my_file.csv' at /Users/user.name/Library/Caches/aiexec/data/my_file.csv
    ```
