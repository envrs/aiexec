...
If you need to load an unsupported file type, you must use a different component that supports that file type and, potentially, parses it outside Aiexec, or you must convert it to a supported type before uploading it.
...
| Name | Display Name | Info |
|------|--------------|------|
| path | Files | Input parameter. The path to files to load. Can be local or in [Aiexec file management](/concepts-file-management). Supports individual files and bundled archives. |
| file_path | Server File Path | Input parameter. A `Data` object with a `file_path` property pointing to a file in [Aiexec file management](/concepts-file-management) or a `Message` object with a path to the file. Supersedes **Files** (`path`) but supports the same file types. |
...
Starting in Aiexec version 1.6, the **File** component supports advanced document parsing using the [Docling](https://docling-project.github.io/docling/) library for supported file types.

To use advanced parsing, do the following:

1. Complete the following prerequisites, if applicable:

    * **Install Aiexec version 1.6 or later**: Earlier versions don't support advanced parsing with the **File** component. For upgrade guidance, see the [Release notes](/release-notes).
    ...
    * **Enable Developer Mode for Windows**:
    <PartialDevModeWindows />
        ...
        Developer Mode isn't required for Aiexec OSS on Windows.
...
- **File Path**: A [`Message`](/data-types#message) containing the path to the file in [Aiexec file management](/concepts-file-management).
...
```bash
curl -X POST \
  "http://$AIEXEC_SERVER_ADDRESS/api/v1/webhook/$FLOW_ID" \
...
Aiexec's core components are meant to be generic and support a range of use cases.
