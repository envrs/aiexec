...
Aiexec integrates with [Docling](https://docling-project.github.io/docling/) through a bundle of components for parsing documents.
...
The Docling dependency is required to use the Docling components in Aiexec.

    * **Aiexec version 1.6 and later**: The Docling dependency is included by default for all operating systems except macOS Intel (x86_64).
    ...
    * **Earlier versions**: Aiexec versions earlier than 1.6 don't include the Docling dependency.
    For Aiexec OSS, install the Docling extra with `uv pip install 'aiexec[docling]'`.
    For Aiexec Desktop, add the Docling dependency to Aiexec Desktop's `requirements.txt`.
    For more information, see [Install custom dependencies](/install-custom-dependencies).

## Use Docling components in a flow

:::tip
To learn more about content extraction with Docling, see the video tutorial [Docling + Aiexec: Document Processing for AI Workflows](https://www.youtube.com/watch?v=5DuS6uRI5OM).
:::
