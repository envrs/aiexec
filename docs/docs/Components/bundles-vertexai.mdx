...
<Icon name="Blocks" aria-hidden="true" /> [**Bundles**](/components-bundle-components) contain custom components that support specific third-party integrations with Aiexec.
...
The following steps explain how to create a service account for Aiexec, create a service account JSON key file, and then use the JSON key file in Aiexec:
...
2. Create a [service account](https://cloud.google.com/iam/docs/service-account-overview) in your Google Cloud project.
    ...
    It is recommended that you [create a custom service account for Vertex AI](https://cloud.google.com/vertex-ai/docs/general/custom-service-account) because Aiexec uses this account to access the Vertex AI APIs.
...
4. [Create a service account key](https://cloud.google.com/iam/docs/keys-create-delete#console) in JSON format for your Aiexec Vertex AI service account.

    When you click **Create**, the JSON key file is downloaded.

5. In Aiexec, create or open a flow with the **Vertex AI** or **Vertex AI Embeddings** component.
...
6. In the component's **Credentials** field, do one of the following:
    ...
    * Leave the **Credentials** field empty if you want to pull the key file from an environment variable.
    Then, you must provide the path to your JSON file in a `GOOGLE_APPLICATION_CREDENTIALS` environment variable that is set somewhere relative to your Aiexec instance.
    For example, you can set the environment variable in your terminal, your Aiexec `.env` file, or otherwise in the environment where your Aiexec server or application runs.

    ![Select a local JSON key file for the Vertex AI component](/img/configure-vertex-ai-credentials-in-aiexec.gif)

7. Run your flow to verify that Aiexec can use the service account credentials successfully.
