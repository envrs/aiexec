...
You can use the **NVIDIA** component to connect Aiexec with NVIDIA NIM on an RTX Windows system with [Windows Subsystem for Linux 2 (WSL2)](https://learn.microsoft.com/en-us/windows/wsl/install) installed.
The following example connects an **NVIDIA** language model component in Aiexec to a deployed `mistral-nemo-12b-instruct` NIM on an **RTX Windows system** with **WSL2**.
...
1. Prepare your system:
    ...
    * It uses the `gassist.rise` package, which is installed with all Aiexec versions that include this component.
