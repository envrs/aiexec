...
For example:
    ...
    If your Aiexec server requires authentication, you must include your Aiexec API key or OAuth settings in the configuration.
...
8. Confirm that your Aiexec MCP server is on the client's list of MCP servers.
...
Auto install requires the client to be installed locally so Aiexec can write to the client's configuration file.
...
1. Install [Cursor](https://docs.cursor.com/get-started/installation), [Claude](https://claude.ai/download), or [Windsurf](https://windsurf.com/download/editor) on the same computer where your Aiexec server is running.
...
Each [Aiexec project](/concepts-flows#projects) has its own MCP server with its own MCP server authentication settings.

To configure authentication for a Aiexec MCP server, go to the **Projects** page in Aiexec, click the **MCP Server** tab, click <Icon name="Fingerprint" aria-hidden="true"/> **Edit Auth**, and then select your preferred authentication method:

<Tabs groupId="auth-type">
<TabItem value="API key" label="API key">

When authenticating your MCP server with a Aiexec API key, your project's MCP server **JSON** code snippets and **Auto install** configuration automatically include the `--headers` and `x-api-key` arguments.
...
When OAuth is enabled, Aiexec automatically starts an [MCP Composer](https://pypi.org/project/mcp-composer) instance for your project, creating a secure client-side proxy between MCP clients and the `mcp-proxy` on your server.

OAuth integration allows your Aiexec MCP server to authenticate users and applications through any OAuth 2.0 compliant service. When users or applications connect to your MCP server, they are redirected to your chosen OAuth provider to authenticate. Upon successful authentication, they are granted access to your flows as MCP tools.

Before configuring OAuth in Aiexec, you must first set up an OAuth application with an external OAuth 2.0 service provider.
You must register your Aiexec server as an OAuth client, and then obtain the required values to complete the configuration in Aiexec.
...
| **MCP Scope** | Scope for MCP operations | You define this. As of Aiexec 1.6, `user` is the only available value. | `user` |
| **Provider Scope** | OAuth provider scope | You define this. As of Aiexec 1.6, `openid` is the only available value. | `openid` |
...
2. Configure the OAuth settings with the values from your OAuth deployment.
    ...
    The OAuth credentials are encrypted and stored securely in your Aiexec database.
...
4. If you already installed your Aiexec MCP server in your MCP client, you must update your MCP client configuration to use the new OAuth settings after enabling OAuth on your MCP server.
...
Only use this option when Aiexec is running in a trusted environment.
...
|----------|--------|---------|-------------|
| `AIEXEC_MCP_SERVER_ENABLED` | Boolean | `True` | Whether to initialize an MCP server for each of your Aiexec projects. If `false`, Aiexec doesn't initialize MCP servers. |
| `AIEXEC_MCP_SERVER_ENABLE_PROGRESS_NOTIFICATIONS` | Boolean | `False` | If `true`, Aiexec MCP servers send progress notifications. |
| `AIEXEC_MCP_SERVER_TIMEOUT` | Integer | `20` | The number of seconds to wait before an MCP server operation expires due to poor connectivity or long-running requests. |
...
3. In the MCP Inspector UI, enter the connection details for your Aiexec project's MCP server.
    ...
    <Tabs groupId="auth-type">
      ...
      <TabItem value="None" label="None">
        ...
        - **URL**: Enter the Aiexec MCP server's `sse` endpoint. For example:
            ```bash
...
For troubleshooting advice for MCP servers and clients, see [Troubleshoot Aiexec: MCP issues](/troubleshoot#mcp).
