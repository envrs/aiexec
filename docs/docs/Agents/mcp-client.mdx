---
title: Use Aiexec as an MCP client
slug: /mcp-client
---
...
This page describes how to use Aiexec as an MCP client with the [**MCP Tools** component](#use-the-mcp-tools-component) and connected [MCP servers](#manage-connected-mcp-servers).

For information about using Aiexec as an MCP server, see [Use Aiexec as an MCP server](/mcp-server).

## Use the MCP tools component

The **MCP Tools** component connects to an MCP server so that a [Aiexec agent](/agents) can use the server's tools when responding to user queries.

This component has two modes, depending on the type of server you want to access:

* [Connect to a non-Aiexec MCP server](#mcp-stdio-mode) with a JSON configuration file, server start command, or SSE URL to access tools provided by external, non-Aiexec MCP servers.
* [Connect to a Aiexec MCP server](#mcp-sse-mode) to use flows from your [Aiexec projects](/concepts-flows#projects) as MCP tools.

### Connect to a non-Aiexec MCP server {#mcp-stdio-mode}
...
2. In the **MCP Server** field, select a previously connected server or click <Icon name="Plus" aria-hidden="true"/> **Add MCP Server**.
    ...
    * **SSE**: Enter your Aiexec MCP server's **Name**, **SSE URL**, and any **Headers** and **Environment Variables** the server uses, and then click **Add Server**.
    ...
    For an example of an `npx` MCP server in Aiexec, see [Connect an Astra DB MCP server to Aiexec](/mcp-component-astra).
    :::

3. To use environment variables in your server command, enter each variable in the **Env** fields as key-value pairs.

    :::tip
    Aiexec passes environment variables from the `.env` file to MCP, but it doesn't pass global variables declared in your Aiexec **Settings**.
    To define an MCP server environment variable as a global variable, add it to Aiexec's `.env` file at startup.
...
### Connect a Aiexec MCP server {#mcp-sse-mode}

Every Aiexec project runs a separate MCP server that exposes the project's flows as MCP tools.
For more information about your projects' MCP servers, including exposing flows as MCP tools, see [Use Aiexec as an MCP server](/mcp-server).
...
2. In the **MCP SSE URL** field, modify the default address to point at your Aiexec server's SSE endpoint. The default value for other Aiexec installations is `http://localhost:7860/api/v1/mcp/sse`.
...
| sse_url | String | Input parameter. SSE mode only. The SSE URL for a Aiexec project's MCP server. Default for Aiexec Desktop: `http://localhost:7868/`. Default for other installations: `http://localhost:7860/api/v1/mcp/sse` |
...
To manage all MCP server connections for your Aiexec client, click <McpIcon /> **MCP servers** in the visual editor, or click your profile icon, select **Settings**, and then click **MCP Servers**.
