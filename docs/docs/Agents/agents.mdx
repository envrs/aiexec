...
Aiexec's [**Agent** component](/components-agents) is critical for building agent flows.
...
The following steps explain how to create an agent flow in Aiexec from a blank flow.
For a prebuilt example, use the **Simple Agent** template or the [Aiexec quickstart](/get-started-quickstart).
...
To continue building on this tutorial, try connecting other tool components or [use Aiexec as an MCP client](/mcp-client) to support more complex and specialized tasks.
...
For more information, see [Add component API keys to Aiexec](/api-keys-and-authentication#component-api-keys).
...
An **Agent** component can use any Aiexec component as a tool, including other agents and MCP servers.
...
Aiexec agents have built-in chat memory that is enabled by default.
...
By default, the **Agent** component uses your Aiexec installation's storage, and it retrieves a limited number of chat messages, which you can configure with the **Number of Chat History Messages** parameter.
