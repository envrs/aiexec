...
This tutorial shows you how to connect a JavaScript application to a [Aiexec agent](/agents).
...
In this example, the application sends a customer's email address to the Aiexec agent. The agent compares the customer's previous orders within the **Directory** component, searches the web for used versions of those items, and returns three results.
...
If not, modify the `const email = "isabella.rodriguez@example.com"` to search for a value in your dataset.
    ...
    const AIEXEC_SERVER_ADDRESS = 'AIEXEC_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';
    const AIEXEC_API_KEY = 'AIEXEC_API_KEY';
    ...
    async function runAgentFlow(): Promise<void> {
        try {
            ...
            const client = new AiexecClient({
                baseUrl: AIEXEC_SERVER_ADDRESS,
                apiKey: AIEXEC_API_KEY
            });

            console.log(`Connecting to Aiexec server at: ${AIEXEC_SERVER_ADDRESS} `);
            ...
            if (error instanceof Error) {
                if (error.message.includes('fetch')) {
                    console.error('\nMake sure your Aiexec server is running and accessible at:', AIEXEC_SERVER_ADDRESS);
                }
    ...
    // Run the function
    console.log('Starting Aiexec Agent...\n');
...
3.  Save and run the script to send the request and test the flow.
    ...
    ```
    Starting Aiexec Agent...

    Connecting to Aiexec server at: http://localhost:7860
