...
The main focus of this tutorial is to show you how to provide files as input to a Aiexec flow, so your chatbot can use the content of those files in its responses.
...
1. In Aiexec, click **New Flow**, and then select the **Basic Prompting** template.
...
To do this, your application must send a `POST /run` request to your Aiexec server with the file you want to upload and a text prompt.
The result includes the outcome of the flow run and the LLM's response.

This example uses a local Aiexec instance, and it asks the LLM to evaluate a sample resume.
If you don't have a resume on hand, you can download [fake-resume.txt](/files/fake-resume.txt).

:::tip
For help with constructing file upload requests in Python, JavaScript, and curl, see the [Aiexec File Upload Utility](https://aiexec-file-upload-examples.onrender.com).
:::
...
1. To construct the request, gather the following information:
    ...
    * `FILE_COMPONENT_ID`: The UUID of the **File** component in your flow, such as `File-KZP68`. To find the component ID, open your flow in Aiexec, click the **File** component, and then click **Controls**. The component ID is at the top of the **Controls** pane.
...
2. Copy the following script into a Python file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    url = "http://AIEXEC_SERVER_ADDRESS/api/v2/files/"
    ...
    # 5. Call the Aiexec run endpoint with the uploaded file path
    run_url = "http://AIEXEC_SERVER_ADDRESS/api/v1/run/FLOW_ID"
    run_payload = {
    ...
    aiexec_data = run_response.json()
    # Output only the message
    message = None
    try:
        message = aiexec_data['outputs'][0]['outputs'][0]['results']['message']['data']['text']
    ...
    The first request uploads a file, such as `fake-resume.txt`, to your Aiexec server at the `/v2/files` endpoint. This request returns a file path that can be referenced in subsequent Aiexec requests, such as `02791d46-812f-4988-ab1c-7c430214f8d5/fake-resume.txt`

    The second request sends a chat message to the Aiexec flow at the `/v1/run/` endpoint.
...
3. Save and run the script to send the requests and test the flow.
    ...
    The initial output contains the JSON response object from the file upload endpoint, including the internal path where Aiexec stores the file.
...
def chat_with_flow(input_message, file_paths):
    ...
    run_url = f"{AIEXEC_SERVER_ADDRESS}/api/v1/run/{FLOW_ID}"
...
To upload files from another machine that isn't your local environment, your Aiexec server must first be accessible over the internet. Then, authenticated users can upload files to your public Aiexec server's `/v2/files/` endpoint, as shown in the tutorial. For more information, see [Aiexec deployment overview](/deployment-overview).
...
In the visual editor, you can preload files to the **File** component by selecting them from your local machine or [Aiexec file management](/concepts-file-management).
