...
This tutorial shows you how to connect MCP servers to your applications using Aiexec's [**MCP Tools** component](/mcp-client).
...
You can run Aiexec as an MCP client and an MCP server:

* [Use Aiexec as an MCP client](/mcp-client): When run as an MCP client, an **Agent** component in a Aiexec flow can use connected components as tools to handle requests.
...
* [Use Aiexec as an MCP server](/mcp-server): When run as an MCP server, your flows become tools that can be used by an MCP client, which could be an external client or another Aiexec flow.

In this tutorial, you will use the Aiexec **MCP Tools** component to connect multiple MCP servers to your flow, and then you'll use a Python application to run your flow and chat with the agent programmatically.

## Prerequisites

* [Install and start Aiexec](/get-started-installation)
* Create a [Aiexec API key](/api-keys-and-authentication)
...
1. In Aiexec, click **New Flow**, and then select the **Simple Agent** template.
...
In Aiexec, you use the **MCP Tools** component to connect a specific MCP server to a flow.
...
1. For this tutorial, install a [weather MCP server](https://github.com/isdaniel/mcp_weather_server) on your local machine with uv and Python:
    ...
    Make sure you install the server in the same Python environment where Aiexec is running:

    * Aiexec in a virtual environment: Activate the environment before installing the server.
    * Aiexec Docker image: Install the server inside the Docker container.
    * Aiexec Desktop or system-wide Aiexec OSS: Install the server globally or in the same user environment where you run Aiexec.
...
4. In the **Add MCP Server** pane, provide the server startup command and arguments to connect the weather MCP server to your flow. For this tutorial, use either the **JSON** or **STDIO** option.
    ...
    Aiexec runs the command to launch the server when the agent determines that it needs to use a tool provided by that server.
...
Note that this tool returns the IP geolocation of your Aiexec server, so if your server is deployed remotely, consider alternative approaches for getting user-specific location data, such as browser geolocation APIs.
...
## Create a Python application that connects to Aiexec
...
In the last part of this tutorial, you'll learn how to use the Aiexec API to run a flow in a script.
...
When you use the Aiexec API to run a flow, you can change some aspects of the flow without changing the code.
For example, you can add more MCP servers to your flow in Aiexec, and then use the same script to run the flow.
...
1. For this tutorial's Python script, gather the following information:
    ...
    * `AIEXEC_SERVER_ADDRESS`: Your Aiexec server's domain. The default value is `127.0.0.1:7860`. You can get this value from the code snippets on your flow's [**API access** pane](/concepts-publish#api-access).
    ...
    * `AIEXEC_API_KEY`: A valid [Aiexec API key](/api-keys-and-authentication).
...
2. Copy the following script into a Python file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    url = "AIEXEC_SERVER_ADDRESS/api/v1/run/FLOW_ID"  # The complete API endpoint URL for this flow
    ...
    Additionally, this script includes parsing code to extract the LLM's reply from the entire Aiexec API response.
    You will want to use similar extraction in your own applications because the Aiexec API response includes metadata and other information that isn't relevant to the reply passed to the user.
...
3.  Save and run the script to send the request and test the flow.
    ...
    Remember, the `ip_geolocation` tool used in this tutorial uses your Aiexec server's location, which can be different from your actual location.
...
* [Use Aiexec Agents](/agents)
* [Use Aiexec as an MCP server](/mcp-server)
* [Aiexec deployment overview](/deployment-overview)