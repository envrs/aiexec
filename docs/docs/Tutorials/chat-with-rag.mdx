...
This tutorial demonstrates how you can use Aiexec to create a chatbot application that uses Retrieval Augmented Generation (RAG) to embed your data as vectors in a vector database, and then chat with the data.
...
* Install the [Aiexec JavaScript client](/typescript-client)
...
1. In Aiexec, click **New Flow**, and then select the **Vector Store RAG** template.
...
In situations where many users load data or you need to load data programmatically, use the Aiexec API option.
...
2. Select the local file you want to upload, and then click **Open**.
    The file is loaded to your Aiexec server.
...
<TabItem value="API" label="Aiexec API">

To load data programmatically, use the `/v2/files/` and `/v1/run/$FLOW_ID` endpoints. The first endpoint loads a file to your Aiexec server, and then returns an uploaded file path. The second endpoint runs the **Load Data Flow**, referencing the uploaded file path, to chunk, embed, and load the data into the vector store.

The following script demonstrates this process.
For help with creating this script, use the [Aiexec File Upload Utility](https://aiexec-file-upload-examples.onrender.com/).
...
const headers = { 'x-api-key': 'AIEXEC_API_KEY' };

// 2. Upload the file to Aiexec
const uploadRes = await fetch('AIEXEC_SERVER_ADDRESS/api/v2/files/', {
  method: 'POST',
  headers,
  body: data
});
...
// 3. Call the Aiexec run endpoint with the uploaded file path
const payload = {
...
const runRes = await fetch('AIEXEC_SERVER_ADDRESS/api/v1/run/FLOW_ID', {
  method: 'POST',
  headers: { 'Content-Type': 'application/json', 'x-api-key': 'AIEXEC_API_KEY' },
  body: JSON.stringify(payload)
});
const aiexecData = await runRes.json();
// Output only the message
console.log(aiexecData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text);
```
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    const { AiexecClient } = require('@datastax/aiexec-client');

    const API_KEY = 'AIEXEC_API_KEY';
    const SERVER = 'AIEXEC_SERVER_ADDRESS';
    const FLOW_ID = 'FLOW_ID';

    const rl = readline.createInterface({ input: process.stdin, output: process.stdout });

    // Initialize the Aiexec client
    const client = new AiexecClient({
        baseUrl: SERVER,
        apiKey: API_KEY
    });
    ...
    function chat() {
        console.log('🤖 Aiexec RAG Chatbot (type "quit" to exit)\n');
    ...
    :::tip
    The [Aiexec TypeScript client](/typescript-client) has a `chatOutputText()` convenience method that simplifies working with Aiexec's complex JSON response structure.
