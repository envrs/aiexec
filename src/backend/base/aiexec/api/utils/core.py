...
from aiexec.utils.constants import AIEXEC_GLOBAL_VAR_HEADER_PREFIX
...
def extract_global_variables_from_headers(headers) -> dict[str, str]:
    """Extract global variables from HTTP headers with prefix X-AIEXEC-GLOBAL-VAR-*.
    ...
    try:
        for header_name, header_value in headers.items():
            ...
            if header_lower.startswith(AIEXEC_GLOBAL_VAR_HEADER_PREFIX):
                var_name = header_lower[len(AIEXEC_GLOBAL_VAR_HEADER_PREFIX) :].upper()
