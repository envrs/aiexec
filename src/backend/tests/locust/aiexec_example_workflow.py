#!/usr/bin/env python3
"""Example Aiexec Load Testing Workflow

This script demonstrates the complete workflow for setting up and running
Aiexec load tests with real starter project flows.
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Example Aiexec Load Testing Workflow")
    parser.add_argument("--auto", action="store_true", help="Run automatically without user input prompts")
    args = parser.parse_args()

    print("🚀 Aiexec Load Testing Example Workflow")
    ...
    setup_script = script_dir / "aiexec_setup_test.py"
    runner_script = script_dir / "aiexec_run_load_test.py"
    ...
    try:
        ...
        success = run_command_with_env(
            [
                ...
                "--no-start-aiexec",
                "--html",
                "aiexec_load_test_report.html",
                "--csv",
                "aiexec_load_test_results",
            ],
        ...
        print("   python setup_aiexec_test.py --interactive")
        print()
        ...
        print("   - aiexec_load_test_report.html (detailed HTML report)")
        print("   - aiexec_load_test_results_*.csv (CSV data files)")
        print("   - aiexec_load_test_detailed_errors_*.log (detailed error logs)")
        print("   - aiexec_load_test_error_summary_*.json (error analysis)")
        print("   - aiexec_server_logs_during_test_*.log (Aiexec server logs)")
        print()
        print("🧹 Clean up:")
        print("   - Remove test flows from Aiexec UI")
        ...
        if args.auto:
            ...
            print("   - aiexec_load_test_report.html")
            print("   - aiexec_load_test_results_*.csv")
        else:
            ...
            if cleanup_response == "y":
                files_to_clean = [
                    "example_test_creds.json",
                    "aiexec_load_test_report.html",
                    "aiexec_load_test_results_failures.csv",
                    "aiexec_load_test_results_stats.csv",
                    "aiexec_load_test_results_stats_history.csv",
                    "aiexec_load_test_results_exceptions.csv",
                ]

                # Also clean up error logs (they have timestamps, so use glob pattern)
                import glob

                error_files = glob.glob("aiexec_load_test_detailed_errors_*.log")
                error_files.extend(glob.glob("aiexec_load_test_error_summary_*.json"))
                error_files.extend(glob.glob("aiexec_server_logs_during_test_*.log"))
        ...
        print("You're now ready to use Aiexec load testing for your own projects.")
        print()
        print("📊 View your load test results:")
        print("   • Open aiexec_load_test_report.html in your browser for detailed analysis")
        print("   • Check aiexec_load_test_results_*.csv for raw data")
        print("   • Review aiexec_load_test_detailed_errors_*.log for comprehensive error details")
        print("   • Analyze aiexec_load_test_error_summary_*.json for error patterns")
        print("   • Examine aiexec_server_logs_during_test_*.log for server-side issues")
