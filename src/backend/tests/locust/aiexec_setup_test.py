#!/usr/bin/env python3
"""Aiexec Load Test Setup CLI

This script sets up a complete Aiexec test environment by:
1. Starting Aiexec (optional)
...
Usage:
    python setup_aiexec_test.py --help
    python setup_aiexec_test.py --interactive
    python setup_aiexec_test.py --flow "Basic Prompting"
    python setup_aiexec_test.py --list-flows
"""
...
async def get_starter_projects_from_api(host: str, access_token: str) -> list[dict]:
    """Get starter projects from Aiexec API."""
    import httpx
...
async def list_available_flows(host: str, access_token: str) -> list[tuple[str, str, str]]:
    """List all available starter project flows from Aiexec API.

    Args:
        host: Aiexec host URL
...
async def setup_aiexec_environment(host: str, flow_name: str | None = None, interactive: bool = False) -> dict:
    """Set up complete Aiexec environment with real starter project flows."""
    try:
        import httpx
    except ImportError:
    ...
    # Configuration - use default Aiexec credentials
    username = "aiexec"
    password = "aiexec"
    ...
    async with httpx.AsyncClient(base_url=host, timeout=60.0) as client:
        ...
        try:
            ...
            # Remove the id to let Aiexec generate a new one
...
def print_setup_results(setup_state: dict):
    ...
    print(f"export AIEXEC_HOST='{setup_state['host']}'")
    ...
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-aiexec --headless --users 25 --duration 120"
    )
    print()

    print("# Generate HTML report:")
    print(
        f"python run_load_test.py --host {setup_state['host']} --no-start-aiexec --headless --users 50 --duration 180 --html report.html"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Set up Aiexec load test environment with real starter project flows",
...
Examples:
  ...
  # Use specific flow
  python setup_aiexec_test.py --flow "Memory Chatbot"

  # List available flows
  python setup_aiexec_test.py --list-flows

  # Setup with custom host
  python setup_aiexec_test.py --host http://localhost:8000 --interactive

  # Save credentials to file
  python setup_aiexec_test.py --interactive --save-credentials test_creds.json
        """,
    )
        ...
        async def list_flows_only():
            ...
            async with httpx.AsyncClient(base_url=args.host, timeout=30.0) as client:
                ...
                try:
                    health_response = await client.get("/health")
                    if health_response.status_code != 200:
                        raise Exception(f"Aiexec not available at {args.host}")
                except Exception as e:
                    print(f"❌ Cannot connect to Aiexec at {args.host}: {e}")
                    sys.exit(1)
                ...
                try:
                    ...
                    print(f"❌ Authentication failed: {e}")
                    print("Make sure Aiexec is running with default credentials (aiexec/aiexec)")
                    sys.exit(1)
    ...
    try:
        ...
        setup_state = asyncio.run(
            setup_aiexec_environment(host=args.host, flow_name=args.flow, interactive=args.interactive)
        )
