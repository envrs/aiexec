"""Aiexec API Locust Load Testing File.

Comprehensive load testing for Aiexec API with multiple user behaviors and performance analysis.
...
TEST_MESSAGES = {
    ...
    "realistic": "Hey, Could you check https://aiexec-docs.khulnasoft.com for me? Later, could you calculate 1390 / 192 ?",
}
...
ERROR_LOG_FILE = None
AIEXEC_LOG_FILE = None
DETAILED_ERRORS = []


def setup_error_logging():
    """Set up detailed error logging for the load test."""
    global ERROR_LOG_FILE, AIEXEC_LOG_FILE

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")

    # Create detailed error log
    ERROR_LOG_FILE = f"aiexec_load_test_detailed_errors_{timestamp}.log"

    # Set up error logger
    error_logger = logging.getLogger("aiexec_load_test_errors")
    ...
    # Try to capture Aiexec logs
    aiexec_log_paths = ["aiexec.log", "logs/aiexec.log", "../../../aiexec.log", "../../../../aiexec.log"]

    for log_path in aiexec_log_paths:
        if Path(log_path).exists():
            AIEXEC_LOG_FILE = log_path
            break

    print("📝 Error logging setup:")
    print(f"   • Detailed errors: {ERROR_LOG_FILE}")
    if AIEXEC_LOG_FILE:
        print(f"   • Aiexec logs: {AIEXEC_LOG_FILE}")
    else:
        print("   • Aiexec logs: Not found (will monitor common locations)")
...
def save_error_summary():
    ...
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    summary_file = f"aiexec_load_test_error_summary_{timestamp}.json"
...
def capture_aiexec_logs():
    """Capture recent Aiexec logs if available."""
    if not AIEXEC_LOG_FILE or not Path(AIEXEC_LOG_FILE).exists():
        return None

    try:
        # Read last 1000 lines of Aiexec log
        with open(AIEXEC_LOG_FILE) as f:
            lines = f.readlines()
            recent_lines = lines[-1000:] if len(lines) > 1000 else lines

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        captured_log_file = f"aiexec_server_logs_during_test_{timestamp}.log"

        with open(captured_log_file, "w") as f:
            f.write("# Aiexec server logs captured during load test\n")
            f.write(f"# Original log file: {AIEXEC_LOG_FILE}\n")
            f.write(f"# Capture time: {datetime.now().isoformat()}\n")
            f.write(f"# Lines captured: {len(recent_lines)}\n\n")
            f.writelines(recent_lines)

        print(f"📋 Aiexec logs captured: {captured_log_file}")
        return captured_log_file

    except Exception as e:
        print(f"⚠️  Could not capture Aiexec logs: {e}")
        return None
...
def on_test_stop(environment, **_kwargs):
    ...
    print(f"\n{'=' * 60}")
    print(f"AIEXEC API LOAD TEST RESULTS - GRADE: {grade}")
    print(f"{'=' * 60}")
...
class BaseAiexecUser(FastHttpUser):
    """Base class for all Aiexec API load testing user types."""
    ...
    def on_start(self):
        ...
        if not self.api_key:
            raise ValueError("API_KEY environment variable is required. Run setup_aiexec_test.py first.")

        if not self.flow_id:
            raise ValueError("FLOW_ID environment variable is required. Run setup_aiexec_test.py first.")
        ...
        try:
            with self.client.post(
                ...
                if response.status_code == 200:
                    try:
                        data = response.json()
                        # Aiexec API success check - look for outputs
                        if data.get("outputs"):
...
class NormalUser(BaseAiexecUser):
...
class AggressiveUser(BaseAiexecUser):
...
class SustainedLoadUser(BaseAiexecUser):
...
class TailLatencyHunter(BaseAiexecUser):
...
class ScalabilityTestUser(BaseAiexecUser):
...
class BurstUser(BaseAiexecUser):
