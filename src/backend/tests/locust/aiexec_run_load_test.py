#!/usr/bin/env python3
"""Aiexec Load Test Runner

This script provides an easy way to run Aiexec load tests.
For first-time setup, use setup_aiexec_test.py to create test credentials.

Usage:
    # First time setup (run once):
    python setup_aiexec_test.py --interactive
...
def check_aiexec_running(host):
    """Check if Aiexec is already running."""
    try:
        import httpx
...
def wait_for_aiexec(host, timeout=60):
    """Wait for Aiexec to be ready."""
    print(f"Waiting for Aiexec to be ready at {host}...")
    start_time = time.time()

    while time.time() - start_time < timeout:
        if check_aiexec_running(host):
            print("✅ Aiexec is ready!")
            return True
        time.sleep(2)

    print(f"❌ Aiexec did not start within {timeout} seconds")
    return False


def start_aiexec(host, port):
    """Start Aiexec server if not already running."""
    if check_aiexec_running(host):
        print(f"✅ Aiexec is already running at {host}")
        return None

    print(f"Starting Aiexec server on port {port}...")

    # Start Aiexec in the background
    cmd = [
        sys.executable,
        "-m",
        "aiexec",
    ...
    if wait_for_aiexec(host, timeout=60):
        return process
    process.terminate()
    return None
...
def run_locust_test(args):
    ...
    locust_file = Path(__file__).parent / "aiexec_locustfile.py"

    # Check for required environment variables
    if not os.getenv("API_KEY"):
        print("❌ API_KEY environment variable not found!")
        print("Run aiexec_setup_test.py first to create test credentials.")
        sys.exit(1)
...
def main():
    parser = argparse.ArgumentParser(
        description="Run Aiexec load tests with automatic setup",
...
Examples:
  ...
  # Run against existing Aiexec instance
  python run_load_test.py --host http://localhost:8000 --no-start-aiexec
  ...
  python run_load_test.py --headless --csv results --users 25 --duration 60
    ...
    parser.add_argument(
        ...
        help="Aiexec host URL (default: http://localhost:7860, use https:// for remote instances)",
    )
    parser.add_argument("--port", type=int, default=7860, help="Port to start Aiexec on (default: 7860)")
    parser.add_argument(
        "--no-start-aiexec",
        action="store_true",
        help="Don't start Aiexec automatically (assume it's already running)",
    )
    ...
    try:
        # Start Aiexec if needed
        if not args.no_start_aiexec:
            if args.host.startswith("https://") or not args.host.startswith("http://localhost"):
                print(f"⚠️  Remote host detected: {args.host}")
                print("   For remote instances, use --no-start-aiexec flag")
                print("   Example: --host https://your-remote-instance.com --no-start-aiexec")
                sys.exit(1)

            aiexec_process = start_aiexec(args.host, args.port)
            if not aiexec_process:
                print("❌ Failed to start Aiexec")
                sys.exit(1)
        # Just check if it's running
        elif not check_aiexec_running(args.host):
            print(f"❌ Aiexec is not running at {args.host}")
            if args.host.startswith("https://"):
                print("   Make sure your remote Aiexec instance is accessible")
            else:
                print("Either start Aiexec manually or remove --no-start-aiexec flag")
            sys.exit(1)
        else:
            print(f"🔗 Using existing Aiexec instance at {args.host}")
            if args.host.startswith("https://"):
        ...
        # Clean up Aiexec process
        if aiexec_process:
            print("\nStopping Aiexec server...")
            aiexec_process.terminate()
            try:
                aiexec_process.wait(timeout=10)
            except subprocess.TimeoutExpired:
                aiexec_process.kill()
            print("✅ Aiexec server stopped")
